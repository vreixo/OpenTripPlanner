...
public abstract class RoutingResource {
     ...
     * example, <code>40.714476,-74.005966</code> or <code>mtanyctsubway_A27_S</code>.
     */
    ...
    /**
     * The minimum time, in seconds, between successive trips on different vehicles. This is
     * designed to allow for imperfect schedule adherence. This is a minimum; transfers over longer
     * distances might use a longer time.
     */
    ...
    /**
     * The list of preferred routes. The format is agency_[routename][_routeid], so TriMet_100 (100
    ...
    /**
     * Whether intermediate stops -- those that the itinerary passes in a vehicle, but does not
     * board or alight at -- should be returned in the response. For example, on a Q train trip from
    ...
    /**
     * The comma-separated list of banned trips. The format is agency_trip[:stop*], so: TriMet_24601
     * or TriMet_24601:0:1:2:17:18:19
     */
     ...
     * station is destroyed, such that no trains can drive through the station anymore. The format
    ...
    /**
     * An additional penalty added to boardings after the first. The value is in OTP's internal
     * weight units, which are roughly equivalent to seconds. Set this to a high value to discourage
     ...
     * Preferred transfers also include timed transfers. The value is in OTP's internal weight
     * units, which are roughly equivalent to seconds. Set this to a high value to discourage
     * transfers that are not preferred. Of course, transfers that save significant time or walking
     * will still be taken. When no preferred or timed transfer is defined, this value is ignored.
     */
    @QueryParam("nonpreferredTransferPenalty")
    protected Integer nonpreferredTransferPenalty;

    /**
     * The maximum number of transfers (that is, one plus the maximum number of boardings) that a
     * trip will be allowed. Larger values will slow performance, but could give better routes. This
     * is limited on the server side by the MAX_TRANSFERS value in
     ...
     * optimistic trips. Reasoning is that it is reasonable to delay a trip start 15 minutes to make
     * a better trip, but that it is not reasonable to delay a trip start 15 hours; if that is to be
    ...
    /**
     * An additional penalty added to boardings at stop where accessibility is not total. The value
     * is in OTP's internal weight units, which are roughly equivalent to seconds. Set this to a
     * high value to discourage these stations. Of course, transfers that save significant time or
     * walking will still be taken.
     */
    @QueryParam("environmentalFactors")
    protected String environmentalFactors;

    /*
     * somewhat ugly bug fix: the graphService is only needed here for fetching per-graph time
     * zones. this should ideally be done when setting the routing context, but at present
     * departure/ arrival time is stored in the request as an epoch time with the TZ already
     * resolved, and other code depends on this behavior. (AMB) Alternatively, we could eliminate
     * the separate RoutingRequest objects and just resolve vertices and timezones here right away,
     * but just ignore them in semantic equality checks.
     */
    @Context
    ...
    protected RoutingRequest buildRequest() throws ParameterException {
        ...
        // The routing request should already contain defaults, which are set when it is initialized
        // or in the JSON
...

        {
            ...
            if (date == null && time != null) { // Time was provided but not date
                ...
                try {
                    // If the time query param doesn't specify a timezone, use the graph's default.
                    // See issue #1373.
        ...
        if (optimize != null) {
            // Optimize types are basically combined presets of routing parameters, except for
            // triangle
            request.setOptimize(optimize);
            if (optimize == OptimizeType.TRIANGLE) {
                if (triangleSafetyFactor == null || triangleSlopeFactor == null
                ...
                if (Math.abs(triangleSafetyFactor + triangleSlopeFactor + triangleTimeFactor
                        - 1) > Math.ulp(1) * 3) {
        ...
        boolean tripPlannedForNow = Math
                .abs(request.getDateTime().getTime() - new Date().getTime()) < NOW_THRESHOLD_MILLIS;
        request.useBikeRentalAvailabilityInformation = (tripPlannedForNow); // TODO the same thing
                                                                            // for GTFS-RT
        ...
        request.environmentalFactorThresholds = EnvironmentalFactorsThresholdsBuilder
                .build(environmentalFactors);
    ...
    /**
     * Take a string in the format agency:id or agency:id:1:2:3:4. TODO Improve Javadoc. What does
     * this even mean? Why are there so many colons and numbers? Convert to a Map from trip --> set
     * of int.
     */

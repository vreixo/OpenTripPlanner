...
public class StreetVertexIndexServiceImpl implements StreetVertexIndexService {
        ...
        public static TemporaryStreetLocation createTemporaryStreetLocation(Graph graph,
                ...
                TemporaryStreetLocation location = new TemporaryStreetLocation(label, nearestPoint,
                        name, endVertex);
            ...
            /* forward edges and vertices */
                        ...
                        if (SphericalDistanceLibrary.distance(nearestPoint, fromv.getCoordinate())
                                ...
                                SphericalDistanceLibrary.distance(nearestPoint, tov.getCoordinate())
                                        < 1) {
                                // no need to link to area edges caught on-end
                                edgeLocation = tov;
        ...
        private static void createHalfLocation(TemporaryStreetLocation base, I18NString name,
                ...
                if (endVertex) {
                        ...
                        temporaryPartialStreetEdge.setElevationProfile(ElevationUtils
                                .getPartialElevationProfile(street.getElevationProfile(), 0,
                                        lengthIn), false);
        ...
        @SuppressWarnings("rawtypes") private void postSetup() {
         ...
         */
        @Override public List<TransitStop> getNearbyTransitStops(Coordinate coordinate,
                double radius) {
                ...
                for (TransitStop v : nearby) {
                        if (SphericalDistanceLibrary.distance(v.getCoordinate(), coordinate)
                                <= radius) {
                                results.add(v);
                        }
                }
        ...
        /**
         * Get all edges With points closer of a coordinate
         */
        @Override public List<Edge> getNearbyEdges(Coordinate coordinate, double radius) {
                Envelope env = new Envelope(coordinate);
                env.expandBy(SphericalDistanceLibrary.metersToLonDegrees(radius, coordinate.y),
                        SphericalDistanceLibrary.metersToDegrees(radius));
                List<Edge> nearby = edgeTree.query(env);
                return nearby.stream()
                        .filter(edge -> Arrays.stream(edge.getGeometry().getCoordinates()).anyMatch(
                                pointCoordinate -> SphericalDistanceLibrary
                                        .distance(pointCoordinate, coordinate) <= radius))
                        .collect(Collectors.toList());
        }

        @SuppressWarnings("unchecked") @Override public List<Vertex> getVerticesForEnvelope(
                Envelope envelope) {
        ...
        @SuppressWarnings("unchecked") @Override public Collection<Edge> getEdgesForEnvelope(
                Envelope envelope) {
        ...
        @SuppressWarnings("unchecked") @Override public List<TransitStop> getTransitStopForEnvelope(
                Envelope envelope) {
        ...
        public StreetVertex getIntersectionAt(Coordinate coordinate) {
                double dLon = SphericalDistanceLibrary
                        .metersToLonDegrees(MAX_CORNER_DISTANCE_METERS, coordinate.y);
                ...
                for (Vertex v : nearby) {
                        if (v instanceof StreetVertex) {
                                ...
                                double distanceMeter = SphericalDistanceLibrary
                                        .fastDistance(coordinate, v.getCoordinate());
        ...
        @Override public Vertex getVertexForLocation(GenericLocation loc, RoutingRequest options,
                boolean endVertex) {
        ...
        @Override public String toString() {
                return getClass().getName() + " -- edgeTree: " + edgeTree.toString()
                        + " -- verticesTree: " + verticesTree.toString();
        }

        @Override public Vertex getSampleVertexAt(Coordinate coordinate, boolean dest) {
